...
- **Cross-profile Copy**: Copy files or folders, switch profiles, and paste into another account or provider. Rust streams through temporary disk with source-change checks and create-only destination writes. See [copy behavior and limits](docs/CROSS_PROFILE_COPY.md).
- **Saved and Scheduled Jobs (next release)**: Save local-folder-to-S3 sync rules, run them manually or at intervals while Brows3 is open, and manage schedules and outcomes in Jobs. Automatic uploads require explicit approval. See [scheduling behavior and limits](docs/SCHEDULED_JOBS.md).
...
`amd64`, `x86_64`, and `x64` refer to the same 64-bit x86 architecture. The
