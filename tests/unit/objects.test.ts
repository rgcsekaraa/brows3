...
test('sequential pagination advances the cursor before React commits and deduplicates objects', async () => {
  vi.mocked(objectApi.listObjects)
    .mockResolvedValueOnce({ ...listing('first'), next_continuation_token: 'one' })
    .mockResolvedValueOnce({ ...listing('second'), next_continuation_token: 'two' })
    .mockResolvedValueOnce(listing('second'));
  const view = renderHook(() => useObjects('bucket', 'us-east-1'));
  await waitFor(() => expect(view.result.current.hasMore).toBe(true));
  await act(async () => { const next = view.result.current.loadMore; await next(); await next(); });
  expect(vi.mocked(objectApi.listObjects).mock.calls.slice(1).map(args => args[4])).toEqual(['one', 'two']);
  expect(view.result.current.data?.objects.map(o => o.key)).toEqual(['first', 'second']);
});
