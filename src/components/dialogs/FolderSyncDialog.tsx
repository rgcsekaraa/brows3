...
import { Alert, Box, Button, FormControlLabel, LinearProgress, Table, TableBody, TableCell, TableHead, TableRow, Typography } from '@mui/material';
...
export default function FolderSyncDialog({ bucket, region, prefix, profileId, onClose }: Props) {
  ...
  async function compare() {
    ...
    try { const result = await syncApi.preview(folder, bucket, region, prefix, profileId); if (alive.current) setPreview(result); }
    catch (e) { if (alive.current) setError(String(e)); }
  ...
  return <BaseDialog open title="Sync local folder to S3" onClose={() => { if (busy !== 'start') onClose(); }} maxWidth="md" actions={<>
    <Button size="small" sx={buttonSx} disabled={busy === 'start'} onClick={onClose}>Close</Button>
    <Button size="small" sx={buttonSx} variant="outlined" disabled={!folder || !!busy} onClick={compare}>{preview ? 'Refresh preview' : 'Preview changes'}</Button>
    ...
    <Box sx={{ display: 'flex', gap: 1, alignItems: 'center', mb: 1 }}>
      <Button size="small" sx={{ ...buttonSx, flexShrink: 0 }} startIcon={<FolderOpen />} variant="outlined" disabled={!!busy} onClick={choose}>Choose folder</Button>
    ...
    {preview && <>
      <Typography variant="body2" sx={{ mb: 1 }}>{preview.new_files} new, {preview.changed_files} changed, {preview.unverified_files} unverified, {preview.unchanged_files} unchanged. Upload: {formatSize(preview.upload_bytes)}.</Typography>
      ...
      {uploads === 0 && <Alert severity="success" sx={{ mb: 2 }}>{preview.entries.length ? 'All local files match. Nothing to upload.' : 'This local folder contains no files. Nothing to upload.'}</Alert>}
